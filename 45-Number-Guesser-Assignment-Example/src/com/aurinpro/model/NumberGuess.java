	...
	public int guessTheNumber(int userInput) {
		for (int i = 1; i <= 6; i++) {
			if (userInput != answer && userInput < answer) {
				System.out.println("Sorry Your Guessed Number " + userInput + " is low. Re-Enter the Higher Number :");
				userInput = sc.nextInt();
			} else if (userInput != answer && userInput > answer) {
				System.out.println("Sorry Your Guessed Number " + userInput + " is high. Re-Enter a lower Number :");
				userInput = sc.nextInt();
			} else {
				System.out.println("Your Guessed Number is Right, The Number is : " + userInput
						+ "\n The correct answer is : " + getNumberls());
				continueGame();
